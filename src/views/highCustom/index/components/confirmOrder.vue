    ...
    <div class="order-content">
      ...
      <div class="cart-table" v-if="!id">
        ...
        <div class="table-item" v-for="(item, index) in cartList" :key="index">
          ...
          <div>
            <textarea
              v-model="item.installSite"
              maxlength="50"
              @blur="checkXML(1, item, 'installSite')"
            ></textarea>
          </div>
          <div>
            <textarea
              v-model="item.remark"
              maxlength="50"
              @blur="checkXML(1, item, 'remark')"
            ></textarea>
          </div>
          <div>
            <textarea
              v-model="item.remarkProduct"
              maxlength="50"
              @blur="checkXML(1, item, 'remarkProduct')"
            ></textarea>
          </div>
          <a
            href="javascript:;"
    ...
    >
      ...
      <div class="selector">
        <div>
          <span>报备状态：</span>
          <el-select
            v-model="proParam.queryType"
            placeholder="请选择"
            @change="queryTypeChangeSale"
            filterable
          >
            <el-option
        ...
        <div>
          <span>项目负责人：</span>
          <el-input
            v-model="proParam.projectLeader"
            @blur="changeLeader"
          ></el-input>
        </div>
  ...
  methods: {
    ...
    async saveOrder() {
      if (this.order.delieryTime) {
        let { year, month, day } = this.until.formatDate(
