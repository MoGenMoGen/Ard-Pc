  ...
  <div class="order" v-loading="loading">
    ...
    <div class="order-content">
        ...
        <div>
          <div style="width: 66%">
            ...
            <div class="choose-man-cont">
              ...
              <el-input
                v-show="showShortTime"
                v-model="order.shortTimeName"
              ...
              <div style="margin-left: 50px;display: -webkit-box">
                ...
                <el-select placeholder="请选择" v-model="networkId" @change="netWorkCH" clearable filterable>
                  <el-option
                    ...
                    :label="item.crmNetworkVo.name +'(' +item.networkType + ')'"
      ...
      <div class="cart-table" v-if="!id">
        ...
        <div class="table-item" v-for="(item, index) in cartList" :key="index">
          <div>{{index+1}}</div>
          <div v-if="item.specName">{{item.productName+'('+item.specName+')'}}</div>
    ...
    <el-dialog width="70%" :visible.sync="dialogShow" @close="closeDialog" :close-on-click-modal="false">
      ...
      <div class="selector">
        <div>
          ...
          <el-cascader
            ...
            v-model="proParam.projectAreaCode"
            @change="changeAddr"
            clearable
            ref="myCascader"
          ></el-cascader>
      ...
      <el-pagination
        ...
        :current-page="proParam.pageNum"
        :page-size="proParam.pageSize"
  ...
  export default {
    data() {
      return {
        ...
        networkId: '',
        id: '',
        queryTypeList: [{cd: 3, nm: "未完结"}, {cd: 7, nm: "全部"}],
        networkList: [],
        networkPicker: "",
    ...
    methods: {
      ...
      checkXML(type, item, index) {
        if (type == 1) {//列表
          if (item[index]) {
            ...
            if (i > -1) {
              this.$alert(this.text.join(',') + '输入框中含有这些禁止符号，请去掉所有禁止符号', '', {
                ...
                callback: () => {
                  ...
                  this.forbidden.forEach(v => {
                    if (this.order.remark.indexOf(v.nm) > -1) {
                      this.order.remark = this.order.remark.replace(RegExp(v.nm, "g"), '')
                    }
                  })
                }
              });
      ...
      async getCartList() {
        this.cartList = await this.api.getCartListByBatch({ids: this.ids});
      ...
      submitNetwork(form) {
        ...
        this.until.postData("/crm/network/api/add", JSON.stringify(this.form2)).then(res => {
          if (res.code == 0) {
            this.msgList = [];
      ...
      changeRadio(e) {
        ...
        this.orderTypeList = this.orderTypeList1.filter(item => ((item.arg2) && (item.arg2.trim() == 'public')))
        if (e == 1) {
          ...
          this.api.getAgentOrdrType(this.userInfo.agentInfoId).then(res => {
            if (res.code === 0) {
              if (res.data.orderType) {
                ...
                this.orderTypeList = this.orderTypeList1.filter(item => gl.indexOf(item.cd) != -1 || ((item.arg2) && (item.arg2.trim() == 'public')))
              } else {
                this.orderTypeList = this.orderTypeList1.filter(item => ((item.arg2) && (item.arg2.trim() == 'public')))
              }
            }
      ...
      changeBuyer() {
        ...
        let filter = this.saleList.filter(item => item.id === this.order.agentId);
      ...
      async getBhOrderList() {
        ...
        if (userInfo1) {
          ...
          if (userInfo.userType === 2) {
            ...
            var extUserIds = userInfo.extUserIds
              ? userInfo.extUserIds
              : "";
          }
        }
      ...
      netWorkCH() {
        if (this.networkId != '') {
          let le = this.networkList.find(value => value.crmNetworkVo.id === this.networkId);
      ...
      async saveOrder() {
        if (this.order.delieryTime) {
          let {year, month, day} = this.until.formatDate(this.order.delieryTime);
        ...
        if (this.order.orderType == 11 || this.order.orderType == '11' || this.order.orderType == "11") {
        ...
        if (this.id) {
          ...
          if (!((this.order.itemList && this.order.itemList.length > 0) || (this.order.cartList && this.order.cartList.length > 0))) {
