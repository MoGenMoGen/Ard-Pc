<template>
  <div class="order-list">
    <div class="title">
      ...
      <button class="button3" v-if="info.status===50 || info.status===99" @click.stop="express">查看物流</button>
      <button class="button3" @click="toDetail">查看详情</button>
      <button class="button2" v-if="info.status===50" @click.stop="confirmReceive">确认收货</button>
      <button class="button2" v-if="info.status===0" @click.stop="cancelOp">取消订单</button>
      <el-button
        v-if="info.status===0 || info.status===60 || info.status===70 || info.status===80"
      ...
      <el-button  type="info" plain @click="exptone (info.id)">订单导出</el-button>
    </div>
        ...
        <li v-for="(item,index) in info.orderItemList" :key="item.id" class="item">
          ...
          <div class="div9">
            <div v-if="item.status!='-1'">
              <!--  <p>{{item.orderType}}</p> -->
              <button
                v-if="item.status===0 || item.status===-2 || item.status===60 || item.status===70 || item.status===80"
                class="button1"
...
item.status===40"-->
    ...
    >
      ...
      <div class="step-two">
          ...
          <div class="product-info">
            <div class="parent-wrap" v-for="(itemProp, index) in stepArr" :key="index">
              <div v-for="(item, index) in itemProp.children" :key="index">
                <div
                  v-if="itemProp.children.length>0 && item.type===1"
                >{{itemProp.name+'-'+item.name}}</div>
                ...
                >
                  <el-option
                    ...
                    :value=" itemChild.code + '_'+itemChild.name+ '_' + itemChild.id"
                  ></el-option>
                </el-select>
              </div>
            </div>
        ...
        <div class="edit-number">
          <span>订单数量：</span>
          <el-input-number v-model="orderNum" @change="handleChange" :min="0"></el-input-number>
        </div>
      </div>
      <!-- footer -->
    ...
    >
      ...
      <div class="input-no" style="margin-top:3%">
        ...
        <el-input-number v-model="currentNum" @change="handleChange" :min="0"></el-input-number>
      </div>
      <span slot="footer" class="dialog-footer">
    ...
    >
      <edit-remark @cancalShow="closeEditRemark" @saveRemark="submitRemark" :form="editRemarkForm"></edit-remark>
    </el-dialog>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
       ...
       */
      exptone (id) {
        window.open('/crm/order/api/exptByOrderId?orderId=' + id + '&buyId=' + this.userInfo.userId )
      },
      ...
      submitFuJian(){
        ...
        this.until.postData("/crm/order/api/updFuJian",JSON.stringify(orderItem)).then(res => {
          if (res.code == 0) {
            this.$message({
              message: "附件修改成功",
              type: "success"
      ...
      updFuJian(item){
        this.$confirm('请确认修改订单吗？修改订单会将状态重置为待审核并且重新审核', '提示', {
          ...
          this.until.get("/crm/orderItem/info/"+item.id).then(res => {
            if (res.code == 0) {
              ...
              for (let i = 0; i < conractFileArr.length; i++) {
                if(conractFileArr[i]!="")
                  this.fileList.push({code: conractFileArr[i], name: "附件"+(i+1), url: conractFileArr[i]})
              }
      ...
      confirmProp: async function () {
        ...
        * */
        ...
        this.stepArr.forEach(value => { //stepArr 是对应的项次的产品应该的属性的集合
...
新选择`
        ...
        for (let index = 0; index < filter[0].productAttributeList.length; index++) {
          let name = this.form[filter[0].productAttributeList[index].attributeKeyCode];
          if (!name) {
            this.$message.error("您还没有填写完产品属性,不能确定！");
            return;
          }
